...
play(){this.audio.play();this.isPlaying=!0;this.ui_playPauseBtn.querySelector('.spectrum-icon').classList.add('fa-pause');this.ui_playPauseBtn.querySelector('.spectrum-icon').classList.remove('fa-play');this.playList[this.currentIndex].element.scrollIntoView({behavior:'smooth',block:'start'});document.title="\u266B"+"  "+this.playList[this.currentIndex].artist+" - "+this.playList[this.currentIndex].title;this._dispatchEvent("onplay",this.playList[this.currentIndex])}
...
self.mute()
else self.unmute()});this.audio.addEventListener('play',function(){self.play()});this.audio.addEventListener('pause',function(){self.pause()});this.audio.addEventListener('timeupdate',function(){if(self._isBuffering){self._isBuffering=!1;self._buffer(!1)}
